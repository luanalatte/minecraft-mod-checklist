...
function build_mod(mod) {
    const URL = mod['url'];
    const SLUG = mod['slug'] || get_slug(url);
    if (!SLUG)
        return null
    ...
    const DESC = mod['desc'];
    const CHECKED = mod['checked'] || false;
    ...
    // Checkbox data
    {
        ...
        // Temporary until presets are implemented.
        if (CHECKED)
    ...
    if (checkbox.dataset.name) {
        ...
        modname.textContent = checkbox.dataset.name;
    ...
    if (checkbox.dataset.url) {
        ...
        let hostname = get_hostname(checkbox.dataset.url);
        if (hostname == 'modrinth') {
...
function details_firefox_fix() {
    ...
    details.forEach(d => {
        d.addEventListener('toggle', () => {
            if (d.open) {
                document.querySelectorAll('details[name="' + d.getAttribute('name') + '"]').forEach(d2 => {
                    if (d2 != d)
                        d2.open = false;
                });
            }
...
function build_modlist(json_data) {
